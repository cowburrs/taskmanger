...
def datetoint(date: datetime, timed=timedelta(0)) -> int:
    return int((date + timed).strftime("%Y%m%d%H%M"))


def timedeltatostr(td: timedelta) -> str:
    ...
    if total_seconds > 86400:
        return f"{sign}{days} Days, {hours} Hours"
    if total_seconds > 3600:
        return f"{sign}{hours} Hours, {minutes} Minutes"
    if total_seconds > 60:
        return f"{sign}{minutes} Minutes, {seconds} Seconds"
    return f"{sign}{seconds} Seconds"


def inttodate(n: int) -> datetime:
    return datetime.strptime(str(n), "%Y%m%d%H%M")
...
def absweek(date: datetime):
    return (date - datetime(1969, 12, 29)).days // 7
